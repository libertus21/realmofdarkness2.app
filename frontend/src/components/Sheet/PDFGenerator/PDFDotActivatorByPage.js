...
const DotFieldItem = React.memo(
  ({ field, index, pageKey, onToggle, onRemove }) => {
...
const PageContent = React.memo(
  ({ pageKey, pageData, pageNumber, onToggle, onRemove, sortNumerically }) => {
...
export default function PDFDotActivatorByPage() {
  ...
  const handleToggleDot = useCallback((pageKey, index) => {
    setDotFieldsByPage((prev) => {
      ...
      updated[pageKey][index] = {
        ...updated[pageKey][index],
        active: !updated[pageKey][index].active,
      };
      return updated;
    });
  }, []);
  ...
  const handleGeneratePDF = useCallback(async () => {
    try {
      ...
      setSuccess(true);
      console.log(
        "PDF generated successfully with custom dot activations by page"
      );
    } catch (err) {
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Typography variant="body1" sx={{ mb: 2 }}>
        Este componente te permite activar campos de dots específicos
        organizados por página en el PDF editable.
      </Typography>
      ...
      {success && (
        <Paper sx={{ p: 2, mb: 2, bgcolor: "success.light" }}>
          <Alert severity="success">
            ...
            <Typography>
              Se mapearon los campos de dots por página correctamente.
            </Typography>
          </Alert>
        </Paper>
      )}
      ...
      {showMappingPanel && (
        <Paper sx={{ p: 2, mb: 2 }}>
          ...
          <Typography variant="body2" sx={{ mb: 2 }}>
            Configura los valores de las habilidades para mapear automáticamente
            los dots.
          </Typography>
                ...
                {[
                  ...
                  >
                    <TextField
                      ...
                      onChange={(e) =>
                        handleUpdateSkillValue(skill, e.target.value)
                      }
          ...
          <Box sx={{ mt: 2 }}>
            ...
            >
              Esto activará automáticamente los dots según los valores de las
              habilidades configuradas arriba.
            </Typography>
          </Box>
        </Paper>
      )}
      ...
      {dotFieldsByPage && (
        <Box>
          ...
          <Paper sx={{ p: 2, mb: 2 }}>
            ...
            >
              {Object.entries(dotFieldsByPage).map(
                ([page, pageData], index) => (
          ...
          <Box sx={{ mt: 2 }}>
            {Object.entries(dotFieldsByPage).map(([page, pageData], index) => (
              <Box
                key={page}
                sx={{ display: selectedPage === index ? "block" : "none" }}
              >
                <PageContent
                  pageKey={page}
                  pageData={pageData}
      ...
      <Paper sx={{ p: 2, mt: 2 }}>
        ...
        <Typography variant="body2" component="div">
          <ul>
            <li>
              Haz clic en "Mapear Dots por Página" para cargar los campos de
              dots
            </li>
            <li>
              Usa "Mostrar Mapeo de Habilidades" para configurar los valores de
              la ficha
            </li>
            <li>Configura los valores de atributos y habilidades (0-5)</li>
            <li>
              Haz clic en "Mapear Dots Automáticamente" para activar según los
              valores
            </li>
            <li>
              <strong>Sistema de Niveles:</strong> Cada habilidad tiene 5 dots
              consecutivos (ej: dot13b, dot14b, dot15b, dot16b, dot17b para
              Dexterity)
            </li>
            <li>
              <strong>Activación por Nivel:</strong> Si Dexterity = 3, se
              activan dot13b, dot14b, y dot15b (los primeros 3 niveles)
            </li>
            <li>
              <strong>Patrones Especiales:</strong> Los patrones "ab" o "ba" se
              activan solo si la habilidad está al máximo (5)
            </li>
