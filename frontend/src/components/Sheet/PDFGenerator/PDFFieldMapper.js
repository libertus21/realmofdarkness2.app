...
export default class PDFFieldMapper {
  ...
  async mapFields() {
    try {
      ...
      const fieldsByPage = {
        page1: { textFields: [], checkBoxes: [], radioGroups: [], dropdowns: [], signatures: [] },
        page2: { textFields: [], checkBoxes: [], radioGroups: [], dropdowns: [], signatures: [] },
        page3: { textFields: [], checkBoxes: [], radioGroups: [], dropdowns: [], signatures: [] },
        page4: { textFields: [], checkBoxes: [], radioGroups: [], dropdowns: [], signatures: [] }
      };
  ...
  determinePage(fieldName) {
    ...
    if (name.includes('character') || name.includes('player') || name.includes('clan') ||
        name.includes('strength') || name.includes('dexterity') || name.includes('stamina') ||
        name.includes('charisma') || name.includes('manipulation') || name.includes('composure') ||
        name.includes('intelligence') || name.includes('wits') || name.includes('resolve') ||
        name.includes('athletics') || name.includes('brawl') || name.includes('craft') ||
        name.includes('drive') || name.includes('firearms') || name.includes('larceny') ||
        name.includes('melee') || name.includes('stealth') || name.includes('survival') ||
        name.includes('animal') || name.includes('etiquette') || name.includes('insight') ||
        name.includes('intimidation') || name.includes('leadership') || name.includes('performance') ||
        name.includes('persuasion') || name.includes('streetwise') || name.includes('subterfuge') ||
        name.includes('academics') || name.includes('awareness') || name.includes('finance') ||
        name.includes('investigation') || name.includes('medicine') || name.includes('occult') ||
        name.includes('politics') || name.includes('science') || name.includes('technology') ||
        name.includes('dot') && (name.includes('1') || name.includes('2') || name.includes('3'))) {
      return 'page1';
    }
    
    // Page 2: Disciplines, powers, advantages, backgrounds
    if (name.includes('discipline') || name.includes('power') || name.includes('advantage') ||
        name.includes('background') || name.includes('merit') || name.includes('flaw') ||
        name.includes('dot') && (name.includes('4') || name.includes('5') || name.includes('6'))) {
      return 'page2';
    }
    
    // Page 3: Blood potency, hunger, humanity, willpower, experience
    if (name.includes('blood') || name.includes('hunger') || name.includes('humanity') ||
        name.includes('willpower') || name.includes('experience') || name.includes('notes') ||
        name.includes('dot') && (name.includes('7') || name.includes('8') || name.includes('9'))) {
      return 'page3';
    }
    
    // Page 4: Appearance, description, other details
    if (name.includes('appearance') || name.includes('description') || name.includes('touchstone') ||
        name.includes('ambition') || name.includes('desire') || name.includes('sire') ||
        name.includes('generation') || name.includes('predator') || name.includes('concept') ||
        name.includes('dot') && (name.includes('0') || name.includes('a') || name.includes('b') || name.includes('c'))) {
      return 'page4';
    }
  ...
  getFieldCount(pageFields) {
    return pageFields.textFields.length + pageFields.checkBoxes.length + 
           pageFields.radioGroups.length + pageFields.dropdowns.length + 
  ...
  async getFieldsForPage(pageNumber) {
    try {
      ...
      throw new Error(`Error getting fields for page ${pageNumber}: ${error.message}`);
    }
  }
